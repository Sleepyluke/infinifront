...
Bottom-left minimap: terrain, entities as colored dots, camera viewport rectangle, click-to-jump.

**Files:**
- Create: `godot/scripts/Minimap.cs`
- Modify: `godot/scripts/Hud.cs` (instantiate inside the HUD layer)

- [ ] **Step 1: `godot/scripts/Minimap.cs`**:
...
{
    private const int Scale = 4; // px per cell
    private SimRunner _runner = null!;
    ...
    public void Init(SimRunner runner, CameraRig camera)
    {
        ...
        // bottom-left anchor, sitting above the toolbar
    ...
    public override void _Draw()
    {
        ...
        // resource nodes (cyan), buildings (player color squares), units (player color dots)
        foreach (var n in w.Nodes)
...
NOTE: reads sim state directly (positions via RenderMath only) — consistent with the view-sync architecture since it's read-only and tick-driven. Camera viewport math assumes `CameraRig` centers on `Position`; verify against the CameraRig implementation from Task 8.

- [ ] **Step 2: Instantiate in `Hud.Init`** (Hud has the CanvasLayer): add parameters or a follow-up call in `Main._Ready` after both camera and HUD exist:
...
- [ ] **Step 3: Build + run. Manual verification:** minimap shows ridge/bases/nodes; dots move during fights; white rectangle tracks the camera and zoom changes; clicking/dragging on the minimap jumps the camera; clicks on the minimap do NOT box-select or issue orders in the world.
...
dotnet test
git add godot
git commit -m "feat: minimap with entity dots, camera rect, click-to-jump"
```
